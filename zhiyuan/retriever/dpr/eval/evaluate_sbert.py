...
# -----------------------------
data_dir = join(zhiyuan_path, "datasets")
beir_dir = join(data_dir, "raw", "beir")

# -----------------------------
# Load model
# -----------------------------
model = DRES(
    models.SentenceBERT(model_save_path),
    batch_size=256,
...
retriever = EvaluateRetrieval(
    model,
    k_values=[1, 3, 5, 10, 100, 300, 500, 1000],
...
if args.dataset_name == "msmarco":
    corpus, queries, qrels = GenericDataLoader(
        join(beir_dir, args.dataset_name)
    ).load(split="dev")
...
for dataset_name in tobe_eval:
    ...
    ndcg, _map, recall, scores = retriever.evaluate(
        qrels, results, retriever.k_values
    )

    # MRR
    if dataset_name not in ["dl2019", "dl2020"]:
    ...
    for metric_name, metric_dict in zip(
        ["NDCG", "MAP", "Recall"], [ndcg, _map, recall]
    ):
        logging.info(f"\n{metric_name}")
    ...
    # Save results
    output_file = join(model_save_path, f"{dataset_name}_results.json")
    with open(output_file, "w") as f:
