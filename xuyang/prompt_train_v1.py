...
def main(args):
    ...
    train_dataloader = DataLoader(train_dataset['train'], shuffle=True, collate_fn=default_data_collator, batch_size=1, pin_memory=True)
    eval_dataloader = DataLoader(train_dataset['test'], collate_fn=default_data_collator, batch_size=1, pin_memory=True)
    ...
    model = get_peft_model(model, peft_config)
    model.to(torch.device(f"cuda:{args.device_idx}" if torch.cuda.is_available() else "cpu"))
...
#         batch_size=1,  # Reduced from args.batch_size
#         pin_memory=True
#     )
...
#     # creating model with 4-bit quantization
#     peft_config = PromptTuningConfig(
#         task_type=TaskType.CAUSAL_LM,
#         prompt_tuning_init=PromptTuningInit.TEXT,
#         num_virtual_tokens=args.num_virtual_tokens,
...
#     # training and evaluation
#     # Model already on device via device_map="auto"
#     original_eval_loss = 999999
#     early_stop_epoch = 0
